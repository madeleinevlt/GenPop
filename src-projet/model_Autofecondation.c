//=============================================================================
//    model_Autofecondation.c : implementation
...
// Simulation
	for (Generation = 0; Generation <= NombreGeneration; Generation++) {
		...
		// Allogamy
		for (i = 0; i < NombreIndividu*(1-TauxAutofecondation); i++) {
		...
		// Autogamy
		for (i = round(NombreIndividu*(1-TauxAutofecondation)); i < NombreIndividu; i++) {
			Mother = T_population__ind_get(ParentPop, global__female);
			Sex = global__herma;
			T_population__child_add(OffspringPop, Mother, Mother, Sex);
		}
